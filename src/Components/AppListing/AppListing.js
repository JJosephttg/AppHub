    ...
    const deleteApp = app => {
        mainAppContext.openMainDialog(dialogProps => 
            <DeleteAppDialog {...{...dialogProps, app: app,
                    onDelete: _ => ipcRenderer.invoke("DBUtility-DeleteApp", app.Id).then(_ => props.onDeleteItem()), 
    ...
    return (
        <div style={containerStyle} className={styles["app-list-container"]}>
            ...
            <div className={styles["app-listing-container"]}>
                {props.appList ? props.appList.map(app => <AppItem deleteHandler={deleteApp} app={app} key={app.Id}/>) : null}
            </div>
